...
flags.DEFINE_integer('microbatches', 40, 'Number of microbatches '
...
def load_mammogram():
    h5f = h5py.File('data/mias_train.h5', 'r')
    X_train = h5f['x'][:]
    Y_train = h5f['y'][:]
    h5f.close()
    h5f = h5py.File('data/mias_test.h5', 'r')
    X_test = h5f['x'][:]
    Y_test = h5f['y'][:]
    h5f.close()
...
def main(_):
  ...
  # load mammogram data
  # train_images, train_labels, test_images, test_labels = load_mammogram()
  ...
  eval_dataset = eval_dataset.batch(len(test_images))
  ...
  # Training loop.
  steps_per_epoch = 280 // FLAGS.batch_size
  start = time.time()
  for epoch in range(FLAGS.epochs):
    ...
    print("Train and test 1 epoch spend: {}s".format(end-start))
